...
//lab3sget.cpp
>>>>>>> 22b3a3a93bc57650dfea67b8babc42e2cc3180eb
...
int scores(char argc[])//int argc, char *argv[])
{
    ...
    BIO *outbio = NULL;
    //Fixed Warning: 
    //invalid conversion from 'const SSL_METHOD* 
    //{aka const ssl_method_st*}' to 'SSL_METHOD* 
    //{aka ssl_method_st*}' [-fpermissive]
    //
    //Previous code was: 
    //SSL_METHOD *method
    //
    ...
    //Fixed Warning : invalid conversion form const to Ssl_Method
    //  const SSL_METHOD *method;
    const SSL_METHOD *method;
	...
	char pagename[256] = "~mnafjan/3350/lab7.php?param=";
	int len = strlen(pagename); int j=0;
	int leng = strlen(argv);

	//int leng = sizeof(*argv)/sizeof(**argv);
	for(int i = len; i < len+leng;i++) {
	pagename[i] = argv[j];
	j++;
	char pagename[256] = "/~nobeid/3350/lab3/projectscores.php?param=";
	int len = strlen(pagename);
	int lenarg = strlen(argc);
	for (int i = len; i < len + lenarg; i++) {
		pagename[i] = argc[i + 1];
	}
    int port = PORT;
    ...
    if (connect(sd, (struct sockaddr*)&addr, sizeof(addr)) == -1) {
        ...
        BIO_printf(outbio, "%s: Cannot connect to host %s [%s] on\
        port %d.\n", 'S', hostname, inet_ntoa(addr.sin_addr), port);
    }
    ...
    //fixed warning:s 'if' clause does not guard... [-Wmisleading-indentation]
    //     if (ret <= 0)
    //     
    //     Previous Code was:  param is: 
   // if (ret <= 0) 
    ...
    //Fixed waring was: this 'if' clause does not guard
    //Previous code was:
    //if (ret <= 0)
	//fprintf(stderr, "ERROR: SSL_write\n"); fflush(stderr);
	if (ret <= 0) {
...
void show_cert_data(SSL *ssl, BIO *outbio, const char *hostname) 
{
	...
	if (BIO_printf(outbio, "Displaying the certificate\
         subject data:\n") < 0)
